...
from functions import (
    config,
    DatabaseManager, CreativeLogin, RIALogin, RIAOnline, RIALogInfo, RIAMsgSend, RIALogCommon,
    GeometryUtils, SystemUtils, EasterEggManager,
    main_ai,
    keys, keys_set,
    get_kook_api_instance, send_kook_message, send_kook_notification,
    create_tables
)
...
# 数据库连接自检
logger.info(f'数据库连接自检：\n创造服数据库：{db_manager.creative_db_exists}，RIA数据库：{db_manager.ria_db_exists}')
...
# 工具函数已移动到utils.py模块中的GeometryUtils类
        ...
        try:
            ...
            try:
                ...
                for player_name, data_info in players.items():
                    ...
                    # 使用安全的数据库操作方法
                    db_manager.add_and_commit(online_record)
...
class BotEventHandler:
    ...
    def handle_chat(this, username: str, message: str, *args) -> None:
        ...
        try:
            ...
            # 保存到数据库
            log_entry = RIALogInfo(
                who_string=username,
                log_string=message,
                t=datetime.datetime.now()
            )
            db_manager.add_and_commit(log_entry)
    ...
    def handle_player_joined(this, player: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 检查是否为创造服玩家
            creative_player = db_manager.creative_session.query(CreativeLogin).filter(
                CreativeLogin.player_name == username
            ).first()
            
            if not creative_player:
                return
            
            # 使用独立session检查是否已有未结束的登录记录
            from sqlalchemy.orm import Session
            from functions.database import engine
            
            temp_session = Session(bind=engine)
            try:
                existing_login = temp_session.query(RIALogin).filter(
                    RIALogin.player_name == username,
            ...
            if not existing_login:
                ...
                db_manager.add_and_commit(new_login)
    ...
    def handle_player_left(this, player: Dict[str, Any]) -> None:
        ...
        try:
            ...
            temp_session = Session(bind=engine)
            try:
                login_record = temp_session.query(RIALogin).filter(
                    RIALogin.player_name == username,
                    RIALogin.logout_time == None
                ).first()
                ...
                current_time = datetime.datetime.now()
                login_duration = current_time - login_record.login_time
                
                # 更新登出时间
                login_record.logout_time = current_time
                try:
                    temp_session.commit()
                except Exception as db_e:
                    temp_session.rollback()
                    logger.error(f'更新登出时间失败: {db_e}')
            finally:
                temp_session.close()
            
            # 如果在线时间超过1分钟，发送通知
            if login_duration > datetime.timedelta(seconds=60):
                hours = login_duration.seconds // 3600
                minutes = (login_duration.seconds // 60) % 60
...
def handle_msg(message):
    if '请求传送到你的位置' in message:
        ...
        if place == 'iron':
            ...
            bot.chat("/m " + playername + f" 你好，我已接受传送请求。{WELCOME_MESSAGE}。这里是-->{place_map_2['main']}<--")
            time.sleep(1.1)
        ...
        bot.chat("/tpaccept")
        time.sleep(0.1)
        bot.chat("/m " + playername + f" 你好，我已接受传送请求。{WELCOME_MESSAGE}。这里是-->{place_map_2[place]}<--")
        send_kook_message(config.KOOK_MAIN_CHANNEL, f'接受来自{playername}的传送请求')
        logger.info(f'接受来自{playername}的传送请求')
    ...
    # bot.lookAt(c)
    # bot.openContainer(blocks['283'])
    # bot.openBlock(blocks['283'])
...
class MessageManager:
    ...
    def send_pending_messages() -> None:
        ...
        try:
            ...
            try:
                # 查询待发送消息
                pending_messages = temp_session.query(RIAMsgSend).all()
    ...
    @staticmethod
    def update_ai_token() -> None:
        """更新AI访问令牌（已废弃，保持兼容性）"""
        logger.info('AI访问令牌更新功能已废弃')
...
def setup_scheduled_tasks():
    ...
    try:
        ...
        timetable_manager.scheduler.add_job(
            MessageManager.update_ai_token,
            'interval',
            days=20,
            id='update_token'
        )
